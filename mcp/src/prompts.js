...
7. **Report** — one summary: what deployed, backup name, disk trend, viewer trend, anything needing a decision.`
  },
  {
...
1. **Scope it** — diagnose_healthz: which services answer at all? A dead service is a server problem (step 2); services all up with a bad symptom is a content/stream problem (step 3).
2. **Dead service** — server_status (is the container even running?) then server_logs {service} for the boot error. Common exits: a fail-fast config rejection after a hand-edited .env (the log names the exact knob — fix via server_set_env, which validates BEFORE applying and reverts on failure), or a full disk (server_disk). A wedged-but-running service usually clears with server_restart {services:[…]} — note it does NOT apply .env changes.
3. **Sick channel** — broadcaster_list_channels to find it, broadcaster_get_channel for state, broadcaster_channel_logs for ffmpeg's last words (usually the answer for a source that won't play). broadcaster_incidents shows fleet-wide patterns — many channels respawning together points at the upstream or the box, not one channel. A channel showing ON AIR but looping the offline slate means the SOURCE died while the pipeline stayed up (the slate is doing its job — fix the input).
4. **Name the symptom** — diagnose_symptom {symptom} maps the operator's words to the curated KB (buffer clamps, disk growth, out-of-scope registrations, store corruption, …) and backs it with docs_search. Read the returned resource URI before acting — the KB pages carry the exact fix and its side effects.
5. **Repeaters** — if viewers in one region degraded while the origin looks healthy: repeater_status {host} for the edge box (compose state, logs, and its opt-in /metrics when the operator enabled the status server; "not enabled" is normal, not a fault).
6. **Fix or escalate** — apply the KB fix if it is reversible and in scope; anything destructive (server_restore, panel_delete_stream, key rotation) gets the operator's explicit go-ahead first, with the evidence and the blast radius stated. End with what happened, why, and what prevents a repeat.`
  },
  {
