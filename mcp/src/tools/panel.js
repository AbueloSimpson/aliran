...
export function registerPanelTools (ctx, h) {
  ...
  def('panel_add_source', {
    ...
    description: 'Add a remote channel-list source materialized as a category of redirect channels. `format` picks the parser: "json" (default) is the provider-JSON shape; "m3u" is a playlist — channel ids are slugged from the entry NAMES (playlist tvg-ids are usually dummies) and #EXTVLCOPT lines import as per-channel playback headers. `groups` filters an m3u by group-title (case-insensitive exact match; omit for every entry). `titleInclude`/`titleExclude` filter an m3u one level down, by the entry NAME: case-insensitive SUBSTRINGS (not regexes) — with `titleInclude` an entry is imported only if its name contains at least one of them, `titleExclude` drops an entry whose name contains any of them and WINS over `titleInclude`; omit both for no name filtering. Each entry must be 2-64 characters and must NOT contain a comma: the comma separates entries, and every surface that shows the list joins it with commas, so a comma inside one entry would re-split into two much broader rules on the next save. `allowCleartext` (default false) lets THIS source import http:// (non-TLS) stream urls — the source-scoped exemption for a provider that serves some streams over cleartext http; manual channels stay https-only, and http only PLAYS where the client permits cleartext. `epg` (m3u only, default FALSE) gives the imported channels a program guide: each playlist entry names its channel in the guide with `tvg-id`, and `epg:true` maps that to `epgId` — the field the EPG SERVICE matches on to deliver the schedule to viewers over P2P. The playlist ALSO declares its guide document in the `#EXTM3U` header (`url-tvg="…"`, aliases `x-tvg-url`/`tvg-url`): the panel reads it and reports it as `epgDeclared` in the sync report, but NEVER writes it onto a channel — it is XMLTV, while the client-side consumer of `epgUrl` JSON.parses what it fetches, so stamping it on records would make every viewer device download a multi-megabyte file it cannot read. Register that address in the EPG service instead. `epgUrl` is a separate, operator-set pointer that IS written to the channels for the apps to fetch, so it must serve the provider-JSON guide shape and https is required (the viewer\'s device fetches it — same rule as a manual channel\'s epgUrl; "" for none, and setting `epg:false` clears it). Both fields are m3u-only and are REFUSED on a json source, which already carries its own guide ids. LEAVE `epg` OFF FOR AN EVENTS PLAYLIST: those write PLACEHOLDER tvg-ids ("Soccer.Dummy.us" on every football match of the day) whose programmes are hourly filler, and the EPG service maps guide-channel → stream by epgId taking the FIRST match on a duplicate — a shared id would collapse a hundred events onto one guide entry. As a guard the panel refuses any tvg-id it finds on more than one imported entry of the same sync, AND any tvg-id an existing channel outside this source already carries (another source, or a manual channel — the incumbent keeps it, the newcomer gets none), counting both as `epgSkipped` in the sync report, so a mistaken opt-in shows up as "no guide" rather than a wrong one. Turn `epg` on for a playlist of ordinary TV channels, where every tvg-id is real and distinct. RECIPE for a playlist that mixes categories: add SEVERAL sources over the SAME url, each with a disjoint `groups` list and its own `category` and `prefix` — ids stay disjoint, each source prunes independently, and the ETag makes the extra fetches nearly free. CHILD-RAIL RECIPE for ONE group that mixes sports in the entry names ("[MLB] Red Sox at Blue Jays", "[NFL] …"): same several-sources-over-one-url shape, but split with disjoint `titleInclude` instead — {titleInclude:["[MLB]"], category:"Live Events/MLB", prefix:"mlb."}, {titleInclude:["[NFL]"], category:"Live Events/NFL", prefix:"nfl."}, and a catch-all {titleExclude:["[MLB]","[NFL]"], category:"Live Events", prefix:"ev."} for the rest. A category of the form "Parent/Child" is already a child rail in the apps, so no client change is needed. Add dead provider tags ("(WEBCAST)") to `titleExclude` to drop entries that never play. AUTOMATIC ALTERNATIVE to that recipe — `autoSubcategory` (m3u only, default FALSE) gets the same rails from ONE source by reading the leading [TAG] off each entry name: "[MLB] Red Sox at Blue Jays" lands in "Live Events/MLB" and "[NFL] …" in "Live Events/NFL", with no sport configured in advance. PREFER IT for an events playlist, because the several-sources recipe goes STALE: the list refreshes every ~30 minutes and the sports in it change through the day, so a hand-written source-per-sport list stops covering what the provider actually carries. It ADDS the second level itself, so pass a SINGLE-LEVEL `category` ({autoSubcategory:true, category:"Live Events", prefix:"ev."}) — a "Parent/Child" category is REFUSED, since deriving onto it would store three levels. Only the FIRST tag is read; an entry with no tag, or one that normalises to nothing, keeps the source\'s own category (the parent rail — the same place the catch-all source used to point), so no channel is ever lost to it. There is no list of known sports on purpose: any leading tag becomes a rail, because a fixed list would go stale exactly the way the manual recipe does. Provider text is normalised (control characters and "/" removed, whitespace collapsed, length bounded so "Parent/Child" fits the 64-character category limit) and at most 50 DISTINCT rails are derived per sync — entries past that go to the parent and are counted as `subcatOverflow`. The sync report lists the rails it derived in `subcats`.',
    inputSchema: { name: z.string(), url: z.string(), category: z.string(), format: z.enum(['json', 'm3u']).optional(), groups: z.array(z.string()).optional(), titleInclude: z.array(z.string()).optional(), titleExclude: z.array(z.string()).optional(), autoSubcategory: z.boolean().optional(), prefix: z.string().optional(), autoGrant: z.boolean().optional(), allowCleartext: z.boolean().optional(), epg: z.boolean().optional(), epgUrl: z.string().optional(), enabled: z.boolean().optional(), intervalMs: z.number().int().optional() }
  }, async (a) => ok(await p.post('/api/sources', a)))

  def('panel_set_source', {
    title: 'Edit a remote source',
    description: 'Edit any field of a remote source. `exclude` REPLACES the deselect list: feed ids (unprefixed, as panel_source_channels reports them) the sync must skip — pass [{id,title}] objects or bare id strings. `groups` REPLACES the m3u group-title filter (empty array = take every group). `titleInclude`/`titleExclude` REPLACE the m3u name filters — case-insensitive substrings of the entry name (2-64 characters each, no commas — see panel_add_source), exclude wins over include, empty array = take every name (see panel_add_source for the child-rail recipe: several sources over ONE playlist url with disjoint `titleInclude`, each with its own "Live Events/MLB"-style category and its own prefix). `allowCleartext` toggles the source-scoped http:// exemption (see panel_add_source). `epg` toggles the m3u guide id (tvg-id → epgId) and `epgUrl` sets the app-fetched, provider-JSON guide pointer (https, "" for none; `epg:false` clears it; the playlist header\'s own XMLTV url is reported as `epgDeclared`, never stored) — see panel_add_source, keep `epg` off for an events playlist whose tvg-ids are placeholders, and note both are refused on a json source. `autoSubcategory` toggles per-[TAG] sub-rails on an m3u (see panel_add_source) — it is judged against the state the edit LEAVES BEHIND, so turning it on while editing `category` to a two-level value is refused, and switching `format` away from m3u clears it. Changing the exclusion set, the `groups` filter, either name filter, the `format`, `autoSubcategory`, `allowCleartext`, `epg` or `epgUrl` resets the source\'s ETag, so the next sync re-pulls the full feed body and re-diffs (channels a narrowed filter no longer matches are removed then, http entries import once the exemption is on, the guide pointers are re-mapped, and the rails are re-derived).',
