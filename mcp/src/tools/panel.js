...
export function registerPanelTools (ctx, h) {
  ...
  def('panel_add_source', {
    ...
    description: 'Add a remote channel-list source materialized as a category of redirect channels. `format` picks the parser: "json" (default) is the provider-JSON shape; "m3u" is a playlist — channel ids are slugged from the entry NAMES (playlist tvg-ids are usually dummies), #EXTVLCOPT lines import as per-channel playback headers, and the entries get no EPG pointer. `groups` filters an m3u by group-title (case-insensitive exact match; omit for every entry). `allowCleartext` (default false) lets THIS source import http:// (non-TLS) stream urls — the source-scoped exemption for a provider that serves some streams over cleartext http; manual channels stay https-only, and http only PLAYS where the client permits cleartext. RECIPE for a playlist that mixes categories: add SEVERAL sources over the SAME url, each with a disjoint `groups` list and its own `category` and `prefix` — ids stay disjoint, each source prunes independently, and the ETag makes the extra fetches nearly free.',
    inputSchema: { name: z.string(), url: z.string(), category: z.string(), format: z.enum(['json', 'm3u']).optional(), groups: z.array(z.string()).optional(), prefix: z.string().optional(), autoGrant: z.boolean().optional(), allowCleartext: z.boolean().optional(), enabled: z.boolean().optional(), intervalMs: z.number().int().optional() }
  }, async (a) => ok(await p.post('/api/sources', a)))

  def('panel_set_source', {
    title: 'Edit a remote source',
    description: 'Edit any field of a remote source. `exclude` REPLACES the deselect list: feed ids (unprefixed, as panel_source_channels reports them) the sync must skip — pass [{id,title}] objects or bare id strings. `groups` REPLACES the m3u group-title filter (empty array = take every group). `allowCleartext` toggles the source-scoped http:// exemption (see panel_add_source). Changing the exclusion set, the `groups` filter, the `format` or `allowCleartext` resets the source\'s ETag, so the next sync re-pulls the full feed body and re-diffs (excluded channels already imported are removed then, and http entries import once the exemption is on).',
