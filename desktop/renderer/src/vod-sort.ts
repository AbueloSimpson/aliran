// VOD list shaping — sorting, the A–Z buckets and the tile label (S54d, design D4/D5/D8).
//
// The DESKTOP MIRROR of client/src/vod/sort.ts: same keys, same labels, same ordering
// rules, same helpers. Keep the two files in step — a viewer who uses both apps must
// get the same grid out of the same catalog. (It is a copy rather than an import for
// the same reason main/vod-provider.js copies client/src/vod/zencontent.ts: the
// desktop bundle cannot reach into the React Native workspace.)
//
// The PROVIDER has exactly one order (newest-added first) and no query API, so every
// ordering the mockup offers is done here, in memory, over the already-downloaded
// list. This module is deliberately pure: no React, no bridge, no provider — screens
// call `sortItems` inside a useMemo.
//
...
import type { VodHistoryEntry, VodItem } from './types'
