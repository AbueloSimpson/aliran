...
export function makeAnalytics ({ dataDir, retentionDays = 90, clock = Date.now } = {}) {
  ...
  // Atomic, so a crash mid-write can never truncate a rollup. indent 0 keeps the compact
  // on-disk form; fsync off — a rollup is disposable and this runs on every tick.
  const writeToday = () => {
