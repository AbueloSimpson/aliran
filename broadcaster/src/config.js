...
// `node src/config.js --check` (the file run DIRECTLY, e.g. via `docker compose
// run --rm broadcaster …`) reports the problem list on stdout and exits 0/1
// instead of throwing — the MCP's server_set_env validates a .env change in the
// built image this way BEFORE restarting. See panel/src/config.js.
const checkRun = process.argv.includes('--check') && process.argv[1] &&
  path.resolve(process.argv[1]).toLowerCase() === fileURLToPath(import.meta.url).toLowerCase()
if (checkRun) {
  if (problems.length) {
    process.stdout.write('broadcaster: invalid configuration —\n  - ' + problems.join('\n  - ') + '\n')
    process.exit(1)
  }
  process.stdout.write('broadcaster: configuration OK\n')
  process.exit(0)
}
