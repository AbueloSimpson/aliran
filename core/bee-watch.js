...
//
// NOT re-exported from index.js on purpose (same reason as net-tune-core.js, store-gc.js and
// atomic-write.js): index.js is the crypto surface, this is infrastructure. Import it by path:
...
// onto it (docs/kb/panel-bee-compaction.md). Every range watch in this tree was armed once and
// never re-armed against that, so one maintenance window left every warm client following a
// bee it could no longer hear — the viewer's live catalog push and grant follow, the repeater's
// catalog and meta follows, the guide's epgId mapping. On a television that is a lineup frozen
// at whatever it said before the window, with no error and no symptom.
//
...
//             against a rebuilt core of ~2,731 — i.e. "until the app is restarted". Measured:
//             re-growing the log 1,006 appends past its pre-fork length did not restore
//             delivery either.
//
// This is why a `while (!closed)` re-arm wrapper — the shape repeater/src/index.js and
// epg/src/guide.js already had — is NOT a fix. It repairs the throwing face only. The parking
// face never leaves the `for await`, so the wrapper never gets to run again.
//
...
// from the tick callback (`onResync`) because they are not always the same work: sdk/player.js
// `_watchGrants` catches up deliberately OUTSIDE its recovery wrapper, since a purge re-arms it
// and recovering from a CATCH-UP could spin purge -> arm -> fail -> purge on a persistently bad
// replica. Splitting the two keeps that doctrine intact across forks as well.
//
// The error path keeps the re-arm loop the two server components already had (a bee hiccup
// must not end an appliance's ability to follow), with a backoff so a permanently unhappy bee
// cannot spin. A fork skips the backoff — a reorg is exactly the moment to be fast.
