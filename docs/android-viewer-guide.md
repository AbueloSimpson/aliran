...
|---|---|
| **Pairing code** | 12 characters in three boxes of four, like `A3K7-9QF2-M4XR` — the boxes move you along as you type |
...
password is what signs you in, and it never leaves your device in readable
form.

Type the code in upper or lower case — either works. If your card shows an
`O` or an `I`, type it as you see it; the app reads them as zero and one.

Some operators hand out a **panel key** instead: 64 letters and digits
(`0–9`, `a–f`). Press **Enter the 64-character panel key instead** and paste
it there. Both routes reach the same service.
...
- Titles **resume** from where you stopped watching. The *Recommended* tab
  starts with a **Continue watching** row, so a title you left is one tap
  away.
- On the phone, the player controls fade out over the video. Tap the screen
  to show them again. Pause keeps them visible. Under the progress bar there
  are **skip buttons** (30 and 10 seconds, in both directions) around
  play/pause, and a **speaker button with a volume slider** — your hardware
  volume keys also work. The **⋮** button selects the subtitle and audio
  languages when the title has them.
