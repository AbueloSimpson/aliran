...
| `reclaimStatus()` | Diagnostics: what the serving core decided about this device's disk bounds — the budgets it measures against, whether each is armed, and the hole-punch capability verdict (`punch: { ok, canPunch, reason, freed, wideFreed }`) that decides whether the blob budget applies here at all. Synchronous, no I/O. `null` before the loopback server exists and after `stop()`. `punch` is `null` until a reclaim tick has **probed** — which needs a feed being served **and** the blob budget enabled, since the probe runs only from that budget's gate: with `reclaimBudgetBytes: 0` there is never a verdict. See §3 `reclaimBudgetBytes`. |
...
signing key, an operator whose device slots are full. None of those is a
judgement on the keys, and the table is the only thing that says which
ones are.
