...
per-user tracking — not as a policy you have to trust, but as an architectural
fact you can verify in the code and on the wire.

## Why the operator cannot see who watches what

In a conventional OTT stack the CDN logs every request, so "who watched what,
when, from where" exists by default and privacy is a promise about what the
operator does with those logs. Aliran's data path makes that record impossible
to produce:
...
- **The broadcaster sees anonymous swarm links.** A connection is an ephemeral
  Noise keypair, not an account. The broadcaster cannot map a peer to a user —
  the panel knows accounts but not peers, the broadcaster knows peers but not
...
> in ephemeral in-memory structures inside the current hour/day; they are reduced
> to **counts** at rollup, and the raw value is never written to any analytics
> file, `/api/analytics` response, or `/metrics` line.

This is not just a design note — `npm run test:analytics` (in the required CI
lane) drives distinctive needle usernames, keys, device ids and IPs through every
path and then scans every rollup file, API response and metrics body for them,
asserting **zero hits**. The one place an identity is touched at all is the
panel's unique-viewers count: usernames of *verified* sessions go into an
in-memory `Set` whose `.size` becomes the day's `uniqueViewers` count; the set
...
downstream of those peers (a repeater measurably absorbs ~half of origin
egress). Every Aliran surface therefore labels peer-derived figures as
**"≥ N"** and never presents them as a viewer count. When you read these
numbers, read them the same way. (Related: the broadcaster's "peer links" tile
also counts a single browsing viewer several times while zap-prefetch holds
neighbouring channels — links, not people, in both directions.)
...
- **`ANALYTICS_RETENTION_DAYS=0` disables collection entirely** — no counters,
  no timers, no files or directories are ever created, `/api/analytics` answers
  `{enabled:false}`, and the per-channel metrics extension disappears. The
  pre-existing health gauges stay.
