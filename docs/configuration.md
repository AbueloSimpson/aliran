...
| `SLATE_RETRY_MS` | `60000` | How often a slated channel drops the slate to re-probe its real source. This is also how it returns automatically: a working source clears the failure streak and stays; a still-dead one re-slates on the next failure |
