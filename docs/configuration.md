...
| `LIVENESS_SCOPE` | `events` | Which channels the probe is allowed to judge. `events` (the default) sweeps **only the Live Events drive** — the population with no other authority over whether it is live, and the one this probe was calibrated for. `all` also sweeps ordinary catalog redirect channels, which is what the probe did before this setting existed. Leave it at `events` unless your lineup really is hand-maintained redirect URLs with no rebuilding pipeline behind it: the panel probes from **one datacenter address**, and the FAST platforms an ordinary lineup is largely built on (Pluto, Samsung TV+, Rakuten, Plex) refuse exactly that traffic — so `all` marks channels offline that play perfectly well for your viewers. A source rebuilt daily by its own pipeline already drops channels that died upstream. **Narrowing the scope does not undim anything** — it stops new verdicts; flags an earlier `all` sweep left behind stay until they are re-stamped. |
...
| `HLS_LIST_SIZE` | `8` | Rolling playlist window, in segments. Use `12` (24 s at `HLS_TIME=2`) as the standard setting: it gives the viewer apps' ~10 s live offset a comfortable margin, and large swarms more blocks to share. Do not go below `8` — the viewer apps sit ~10 s behind the live edge. |
