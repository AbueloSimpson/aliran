...
| `vod-config` / `vod-config-set [--enabled --api-base --service --movies-source --series-source --params "hm=1,hs=2" --param hs=2]` | Show / configure the external VOD provider the **apps** call directly (`--params` replaces the map, `--param` merges one key; each `--*-source` merges its kind, `""` clears it — no series source = the apps show movies only). Needs the store: panel stopped, or use the dashboard/API |
