...
| `vod-config` / `vod-config-set [--enabled --api-base --service --movies-source --params "hm=1,hs=2" --param hs=2]` | Show / configure the external VOD provider the **apps** call directly (`--params` replaces the map, `--param` merges one key). Needs the store: panel stopped, or use the dashboard/API |
...
| `GET/PATCH /api/vod-config` | External VOD provider (S53): the replicated `svcmeta/vod` record (`null` when never configured) / partial merge of `{enabled,apiBase,service,sources,params}`. `apiBase` is https with **no query string and no embedded credentials**; `sources` (only `movies` today) and `params` replace their whole map; `enabled:true` is refused without an apiBase + service. Viewers pick a change up at their **next login** |
