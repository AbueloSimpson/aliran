// Library ⇄ panel registration link.
//
// Self-contained copy of broadcaster/src/panel-link.js (S15b) — separate deployables
// each ship their own copy (same convention as control-auth.js). If you fix a bug here,
// fix it there. The library rides it unchanged: a title registration is a stream
// registration (`type:'vod'` in the payload, no `isLive`), and every PanelLink behavior
// — serialized register cycles, terminal policy rejects, the relookup self-heal, the
// 5-min idempotent heartbeat — applies to a VOD seeder exactly as to a live channel.
// (Heartbeat note: payloads without `isLive` count as running, which is right — a ready
// title IS a running seed; a delete op carries `isLive:false` purely to stop its own
// heartbeat, the panel's vod record class never reads isLive.)
//
