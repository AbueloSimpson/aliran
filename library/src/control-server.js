// Aliran library control API — authed HTTP+JSON over the TitleManager.
// Self-contained adaptation of broadcaster/src/control-server.js (same skeleton as the
// panel admin server) — separate deployables each ship their own copy; if you fix a
// bug in the skeleton, fix it in all three.
//
// Auth: POST /api/login against DATA_DIR/secrets/admins.json (library-cli add-admin)
// → session token signed with the library-local control keypair. Every other /api
// route requires `Authorization: Bearer <token>`. Login is rate-limited AND
// single-flight: the Argon2id verify runs in a worker thread (never on the event
// loop), one verify at a time, concurrent attempts get an immediate 503.
// Binding: 127.0.0.1 by default; put TLS in front if you expose it.
//
//   POST   /api/login                    {username,password} → {token,expiresAt}
//   GET    /api/status
//   GET    /api/titles                   list + state/progress/peers/registered
//   POST   /api/titles                   {id,input,title?,description?,category?,protection?,mode?,hlsTime?}
//                                          → creates the title and QUEUES its one-shot ingest
//   GET    /api/titles/:id               registry view incl. live ingest {phase,pct}
//   PATCH  /api/titles/:id               input/mode/hlsTime (descriptive metadata is
//                                          panel-owned after creation — edit it there)
//   POST   /api/titles/:id/ingest        {input?} re-ingest: next feed generation
//   DELETE /api/titles/:id               stop seeding + purge from disk (refused mid-ingest)
//   GET    /api/titles/:id/logs?lines=N  ffmpeg/ingest log ring → {lines,state}
//   GET    /api/admins
...
//
// GET /healthz is UNAUTHENTICATED liveness (cheap, synchronous — same contract as the
// broadcaster's). Non-/api GETs serve the minimal control UI from library/control-ui/.
...
export function startControlServer (ctx, opts = {}) {
  ...
  async function handle (req, res) {
    ...
    // Liveness. Unauthenticated and handled FIRST — cheap and synchronous, so it
    // answers as long as the event loop turns at all (even mid-transcode).
    ...
    if (r1 === 'titles') {
      if (seg.length === 2) {
        if (req.method === 'GET') return sendJson(res, 200, ctx.manager.list())
        if (req.method === 'POST') {
          const b = await readJson(req)
      ...
      if (seg.length === 3) {
        if (req.method === 'GET') return sendJson(res, 200, ctx.manager.get(r2))
      ...
      if (seg.length === 4 && r3 === 'ingest' && req.method === 'POST') {
        return sendJson(res, 200, await ctx.manager.reingest(r2, await readJson(req)))
      }
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(opts.port ?? 3320, host, () => {
...
// --- helpers (same never-throw rationale as the panel/broadcaster copies) ---
