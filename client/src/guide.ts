// Guide-grid logic (WS3): geometry + the virtual-focus reducer for the full EPG
// guide screen. Pure functions over the SDK's EpgProgram (epoch-ms start/stop —
// see sdk/react-native/src/epg.ts), react-free so jest covers them like catalog.ts.
//
// The TV grid never moves NATIVE focus cell-to-cell — react-native-tvos on Android
// does not dispatch HW key events while a view holds focus (the S7 lesson), so the
// screen keeps native focus parked on a catcher and drives a VIRTUAL focus through
// moveFocus below. Keeping the reducer pure keeps every D-pad rule testable without
// a TV.
