// Replica-store corruption recovery — runtime-agnostic (runs in Bare and in Node tests).
//
...
// wrapped or re-thrown and lost their code.
const CORRUPT_CODES = ['OPLOG_CORRUPT', 'OPLOG_HEADER_OVERFLOW', 'INVALID_OPLOG_VERSION', 'INVALID_CHECKSUM', 'DECODING_ERROR']

export function isCorruptionError (err) {
  if (!err) return false
  if (CORRUPT_CODES.indexOf(err.code) >= 0) return true
  return /corrupt/i.test(String(err.message || err))
}
