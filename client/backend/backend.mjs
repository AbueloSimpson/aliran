...
//
// IPC (line-delimited JSON) with React Native — UNCHANGED by the SDK extract:
...
function ensurePlayer () {
  ...
  player = new AliranPlayer({ storeDir: storeDir(), http, fs })
...
IPC.on('data', (data) => {
  ...
  while ((i = buf.indexOf('\n')) >= 0) {
    ...
    if (msg.feedKey && msg.encryptionKey) {
      ...
      ensurePlayer().resolve(msg.streamId).then(({ port }) => send({ type: 'port', port })).catch(fail)
    } else if (msg.panelPubKey) {
      ensurePlayer().connect(msg.panelPubKey).catch(fail)
    }
  }
})
